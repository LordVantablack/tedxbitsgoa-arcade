      ...
      <section className="leaderboard-page__grid" aria-label="Game leaderboards">
        {Object.values(GAMES).map((game, index) => (
          <article className="leaderboard-card" key={game.id}>
            ...
            {boards[game.id]?.viewer ? <p className="leaderboard-card__viewer"><span>YOUR VERIFIED PB · #{boards[game.id]?.viewer?.rank}</span><strong>{boards[game.id]?.viewer?.score}</strong></p> : <p className="leaderboard-card__viewer quiet">SIGN IN TO SEE YOUR VERIFIED PB.</p>}
          </article>
        ))}
      </section>
    </main>
